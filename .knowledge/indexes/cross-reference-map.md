...
This document is the cross-reference map for the `.knowledge/` knowledge base. It defines the directed relationships between all research nodes. Use this to traverse the research graph: given a starting node, this document lists every node it links to and every node that links to it. For the full machine-readable version, see `map.json` in the `.knowledge/` root. For node descriptions and domain groupings, see [[master-index]].
