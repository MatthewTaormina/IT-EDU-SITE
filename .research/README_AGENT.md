...
**Scope:** Any AI agent reading or writing data within the `.research/` directory  

---

## 1. WHAT THIS FOLDER IS

`.research/` is an **Instructional Knowledge Engine** — a machine-optimized research graph that backs IT educational content generation for the IT EDU SITE platform. It is **not** public-facing documentation. It is **not** an application. It exists exclusively to:
...
| :--- | :--- | :--- |
| 1 | `.research/map.json` | **Always read first.** Gives the full node graph and edge relationships in one JSON parse. Use to identify relevant documents before reading any Markdown file. |
| 2 | `.research/validation/gap-analysis.md` | Read when the task involves new content creation — check the gap register before generating anything that might already be covered or that fills a CRITICAL gap. |
...
4. CHECK .research/validation/gap-analysis.md → verify the topic is not a CRITICAL gap that needs a new research document first
...
```
1. READ .research/map.json → find the target node
...
```
1. READ .research/validation/gap-analysis.md → extract gap_summary from manifest.json
2. FILTER: severity = CRITICAL || HIGH
3. CROSS-REFERENCE: .research/map.json → does a node already exist for this gap?
...
4. UPDATE .research/map.json: add the new node and its edges
5. UPDATE the domain manifest.json: add the document entry with terminal_objective and educational_assets
6. UPDATE .research/indexes/master-index.md: add the node to the appropriate domain table
```
...
| **No public-facing content** | Everything in `.research/` is internal background data |
