...
/**
 * One focused sidebar per course / project keeps the navigation panel lean.
 *
 * Catalog pages (Pathways/index, Courses/index, Projects/index, index) are
 * deliberately excluded from every sidebar so they render without a sidebar —
 * they are browse / landing pages, not reading-flow pages.
 *
 * webdevCourseSidebar
 *   Shown while reading the Front-End Basics (web) course. Starts at the course
 *   overview and sequences all units → sub-units → lessons → reviews in
 *   depth-first order so the "Next" button always descends before advancing.
 *   The capstone project is referenced as a plain link (not a doc node) so
 *   project pages keep their own focused sidebar.
 *
 * webdevProjectSidebar
 *   Shown while working through the Capstone — Personal Portfolio project.
 *
 * articlesSidebar
 *   Standalone articles; autogenerated from the Articles/ directory.
 */
const sidebars: SidebarsConfig = {
  // ── Front-End Basics (web) course ─────────────────────────────────────────
  webdevCourseSidebar: [
    {
      type: 'category',
      label: 'Front-End Basics (web)',
