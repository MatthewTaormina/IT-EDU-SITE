---
type: course
title: "Front-End Basics (web)"
description: "From the theory of how the web works to building styled, interactive front-end pages — HTML, CSS, and JavaScript fundamentals."
domain: "WebDev"
difficulty: "Beginner to Intermediate"
prerequisites:
  - "None for the first unit. Later units assume completion of prior units."
references:
  - type: unit
    slug: webdev_00_web_foundations
  - type: unit
    slug: webdev_01_html
  - type: unit
    slug: webdev_02_css
  - type: unit
    slug: webdev_03_js_basics
  - type: project
    slug: webdev_capstone_portfolio
---

# Front-End Basics (web)

> **Course Summary:** This course takes you from the foundational theory of how the web works to writing styled, interactive front-end pages. You will understand the systems beneath your code before you write a single line of it — then build mastery in HTML, CSS, and JavaScript from the ground up.

## Course Objectives

By the end of this course, you will be able to:

- Explain how browsers, servers, DNS, and HTTP interact to deliver a web page
- Write semantic, accessible HTML documents
- Style pages with CSS using the box model, layouts, and responsive design
- Write JavaScript that manipulates the DOM and handles user events
- Build and deploy a complete, interactive static front-end project

## Prerequisites

None. This course starts from zero.

---

## Units

### Unit 1 — Web Foundations
The theory of the web: how browsers, servers, DNS, and HTTP work before you write any code.
→ [Start Unit](../../Units/webdev_00_web_foundations/index.md)

### Unit 2 — HTML
The structural language of the web. Document structure, the DOM, text, links, images, lists, tables, forms, semantic landmarks, the `<head>`, and accessibility basics.
→ [Start Unit](../../Units/webdev_01_html/index.md)

### Unit 3 — CSS
Cascade, box model, selectors, Flexbox, Grid, typography, colour, responsive design.
→ [Start Unit](../../Units/webdev_02_css/index.md)

### Unit 4 — JavaScript Basics
Variables, types, control flow, functions, data structures, DOM manipulation, and events.
→ [Start Unit](../../Units/webdev_03_js_basics/index.md)

---

## How to Use This Course

- **Work in order.** Each unit builds on the last. Skipping foundations creates gaps.
- **Use the Research Questions.** They point beyond the lesson — follow them when you're curious.
- **Do the Challenges.** Reading is not enough. Every concept must be applied.
- **Open DevTools.** Keep your browser developer tools open as you learn. Verify everything yourself.
